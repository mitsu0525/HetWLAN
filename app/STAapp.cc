...
Define_Module(STAapp);

simsignal_t STAapp::sentPkSignal = registerSignal("sentPk");
simsignal_t STAapp::rcvdPkSignal = registerSignal("rcvdPk");
simsignal_t STAapp::endToEndDelaySignal = registerSignal("endToEndDelay");
...
void STAapp::initialize(int stage)
{
...
void STAapp::handleMessage(cMessage *msg)
{
    if (!isNodeUp())
...
bool STAapp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
bool STAapp::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

bool STAapp::isGenerator()
{
    return par("destAddress").stringValue()[0];
}

void STAapp::scheduleNextPacket(bool start)
{
...
void STAapp::cancelNextPacket()
{
    if (timerMsg)
        cancelEvent(timerMsg);
}

MACAddress STAapp::resolveDestMACAddress()
{
...
void STAapp::registerDSAP(int dsap)
{
...
void STAapp::receivePacket(cPacket *msg)
{
